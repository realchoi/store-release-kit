...
- `packages/translators`：依赖 core，定义 `TranslatorProvider`，第一版实现 mock provider，并保留 OpenAI / DeepL skeleton。
- `packages/adapters`：依赖 core，定义 `StoreAdapter`，第一版提供 mock adapter、Fastlane export、App Store Connect skeleton。
